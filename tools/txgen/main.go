...
func parseTarget(dir string, targetName string) (*targetInfo, error) {
	...
	pkgs, err := parser.ParseDir(fset, dir, func(fi os.FileInfo) bool {
		name := fi.Name()
		return !strings.HasSuffix(name, "_test.go") && !strings.HasSuffix(name, "_tx.go")
	...
	availableImports := make(map[string]string) // localPkgName -> importPath
	...
	for _, field := range foundInterface.Methods.List {
		...
		isTx := strings.Contains(docText, "@transactional") ||
			strings.Contains(docText, "@tx") ||
			strings.Contains(docText, "//tx:wrap")
		...
		if isTx {
			if len(params) == 0 || !strings.Contains(params[0].Type, "context.Context") {
				return nil, fmt.Errorf("method %s is marked @transactional but does not accept context.Context as first argument", mName)
			}
			if len(results) == 0 || results[len(results)-1].Type != "error" {
				return nil, fmt.Errorf("method %s is marked @transactional but does not return error as last return value", mName)
			}
		}
...
func generateDecorator(info *targetInfo) ([]byte, error) {
	...
	decoratorName := "Transactional" + info.TargetName

	buf.WriteString("// Code generated by txgen. DO NOT EDIT.\n\n")
	buf.WriteString(fmt.Sprintf("package %s\n\n", info.PkgName))

	// Collect all imports
	importPaths := make(map[string]string) // path -> alias
	importPaths["context"] = ""
	importPaths["github.com/masterkeysrd/saturn/internal/platform/db"] = ""
	...
	// Struct definition
	buf.WriteString(fmt.Sprintf("// %s wraps a %s and manages database transactions\n", decoratorName, info.TargetName))
	buf.WriteString("// for methods annotated with @transactional.\n")
	buf.WriteString(fmt.Sprintf("type %s struct {\n", decoratorName))
	buf.WriteString(fmt.Sprintf("\tnext %s\n", info.TargetName))
	buf.WriteString("\ttxr  db.Transactor\n")
	buf.WriteString("}\n\n")

	// Constructor
	buf.WriteString(fmt.Sprintf("// New%s creates a new %s decorator.\n", decoratorName, decoratorName))
	buf.WriteString(fmt.Sprintf("func New%s(next %s, txr db.Transactor) *%s {\n", decoratorName, info.TargetName, decoratorName))
	buf.WriteString(fmt.Sprintf("\treturn &%s{\n", decoratorName))
	buf.WriteString("\t\tnext: next,\n")
	buf.WriteString("\t\ttxr:  txr,\n")
	buf.WriteString("\t}\n")
	buf.WriteString("}\n\n")
	...
	for _, m := range info.Methods {
		...
		if !m.IsTransactional {
			...
			if len(m.Results) == 0 {
				buf.WriteString(fmt.Sprintf("func (t *%s) %s(%s) {\n", decoratorName, m.Name, paramsSig))
				buf.WriteString(fmt.Sprintf("\tt.next.%s(%s)\n", m.Name, strings.Join(callArgs, ", ")))
				buf.WriteString("}\n\n")
			} else {
				buf.WriteString(fmt.Sprintf("func (t *%s) %s(%s) %s {\n", decoratorName, m.Name, paramsSig, retSig))
				buf.WriteString(fmt.Sprintf("\treturn t.next.%s(%s)\n", m.Name, strings.Join(callArgs, ", ")))
				buf.WriteString("}\n\n")
			}
			continue
		}
		...
		buf.WriteString(fmt.Sprintf("// %s executes next.%s inside a database transaction.\n", m.Name, m.Name))
		buf.WriteString(fmt.Sprintf("func (t *%s) %s(%s) %s {\n", decoratorName, m.Name, paramsSig, retSig))
		buf.WriteString(fmt.Sprintf("\t%s, tx, err := t.txr.Begin(%s)\n", ctxArgName, ctxArgName))
		...
		if len(m.Results) == 1 {
			buf.WriteString(fmt.Sprintf("\tif err := t.next.%s(%s); err != nil {\n", m.Name, strings.Join(callArgs, ", ")))
			buf.WriteString("\t\treturn err\n")
			buf.WriteString("\t}\n\n")
			...
			buf.WriteString(fmt.Sprintf("\tres, err := t.next.%s(%s)\n", m.Name, strings.Join(callArgs, ", ")))
			buf.WriteString("\tif err != nil {\n")
			buf.WriteString(fmt.Sprintf("\t\treturn %s, err\n", zeroReturns[0]))
			buf.WriteString("\t}\n\n")
			buf.WriteString("\tif err := tx.Commit(); err != nil {\n")
