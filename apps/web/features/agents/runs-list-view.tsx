...
export function AgentRunsListView() {
  ...
  const handleNextPage = () => {
    if (runsData?.nextPageToken) {
      setTokenHistory((prev) => [...prev, pageToken])
      setPageToken(runsData.nextPageToken)
    }
  }
  ...
  // Logs query with cursor pagination
    ...
    <TooltipProvider>
      ...
      >
        <div className="space-y-6">
          ...
          <Card size="sm" className="max-w-4xl overflow-hidden py-0">
              ...
              <div className="space-y-2 p-4">
                {isLoading ? (
                  Array.from({ length: 5 }).map((_, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between rounded-2xl border border-border/20 p-3.5"
                    >
                  ...
                  <div className="rounded-3xl border border-dashed border-border/30 bg-card/10 px-4 py-14 text-center font-sans text-muted-foreground select-none">
                    <History className="mx-auto mb-2 h-8 w-8 text-muted-foreground/60" />
                    <h4 className="text-sm font-bold text-foreground">
                      No Run Records Found
                    </h4>
                    <p className="mx-auto mt-1 max-w-xs text-xs leading-relaxed text-muted-foreground">
                      Select an active agent instance or trigger processing to
                      record execution logs.
                    </p>
                  </div>
                ) : (
                  runsData.runs.map((r: AgentRun) => (
                    <div
                      key={r.id}
                  ...
                  <PaginationContent>
                    ...
                    <PaginationItem>
                      <Button
                        ...
                        disabled={!runsData?.nextPageToken}
                        className="h-8 gap-1 text-xs"
                      >
                        Next
